...
public class RoomParametersFetcher {
  ...
  private void roomHttpResponseParse(String response) {
    ...
    try {
      ...
      if (!isTurnPresent && !roomJson.optString("ice_server_url").isEmpty()) {
        String ice_server_url = roomJson.getString("ice_server_url");
        List<PeerConnection.IceServer> turnServers;

        Log.d(TAG, "ice_server_url: " + ice_server_url);
        
        if (ice_server_url.contains("www.webrtcserver.cn")) {
          turnServers = requestFixedTurnServers(roomJson.getString("ice_server_url"));
        } else {
          turnServers = requestTurnServers(roomJson.getString("ice_server_url"));
        }
  ...
  private List<PeerConnection.IceServer> requestFixedTurnServers(String url)
      throws IOException, JSONException {
    List<PeerConnection.IceServer> turnServers = new ArrayList<>();
    Log.d(TAG, "Request TURN from prepared string");

    String response = "{\"iceServers\":[{\"urls\":[\"turn:47.96.145.81:3478?transport=udp\",\"turn:47.96.145.81:3478?transport=tcp\"],\"username\":\"1682575652:webrtcserver\",\"credential\":\"W9AnwzRX3qZhb5Oai1pp43olY3c=\"},{\"urls\":[\"stun:47.96.145.81\"]}]}";
  ...
  private List<PeerConnection.IceServer> requestTurnServers(String url)
    ...
    if (responseCode != 200) {
      throw new IOException("Non-200 response when requesting TURN server from (by Eason) " + url + " : "
